# ********************************************************
# *  This is a modified version of the "gpsd" daemon     *
# *  that exports the raw satellite location data from   *
# *  the 'SKY'satalite object .                          *
# ********************************************************
...
class GpsResponse(object):
    ...
    def from_json(cls, packet):
        ...
        if 'satellites' in last_sky:
            result.satData = last_sky['satellites']
    ...
    def get_time(self, local_time=False):
        ...
        time = now.strftime(gpsTimeFormat)
        if local_time:
            now = datetime.datetime.now()
            time = now.strftime(gpsTimeFormat)
        return time
...
def connect(host="127.0.0.1", port=2947):
    ...
    try:
        ...
        print("Connection to GPS failed!", "\r\n", "Have you started gspd?")
        sys.exit(1)
